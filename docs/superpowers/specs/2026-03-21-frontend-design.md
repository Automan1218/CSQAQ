...
type ServerMessage =
  ...
  | { type: "result"; task_id: string; payload: QueryResult }
...
用户输入查询
  ...
  → 推送结果 { type: "result", payload: { intent, summary, action_detail, risk_level, requires_confirmation, contexts: { item, market, scout, inventory } } }
```
...
| GET | `/api/v1/items/:id/chart` | K线数据 | ItemAPI.get_item_kline |
...
| WS | `/ws/analysis` | 分析查询推送 | run_query (LangGraph) |
...
async def analysis_handler(websocket, app):
    while True:
        ...
        if msg["type"] == "query":
            ...
            # 利用 LangGraph stream events 推送进度
            async for step in run_query_with_progress(app, msg["payload"]["text"]):
...
- WebSocket 端点封装 `run_query`，增加进度回调（通过 LangGraph stream events）
...
后续请求:
  ...
  - WebSocket: ws://host/ws/analysis?token=xxx

Token 过期:
  - axios interceptor 捕获 401 → refresh_token 刷新 → 重试
  - refresh 也过期 → 跳转登录页
```
